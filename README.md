...
2. Copy `.env.example` to `.env.local` and set values:
   ...
   - `VITE_ANNOUNCEMENTS_URL` … GAS Webアプリの URL（LPおしらせ）
3. Run the app:
   `npm run dev`
...
3. おしらせ用も追加:
   - **Name:** `VITE_ANNOUNCEMENTS_URL`
   - **Value:** GAS のおしらせURL（`https://script.google.com/macros/s/.../exec?page=announcements`）
   - **Environment:** Production（Preview も使う場合は同様に追加）
4. **Deployments** から **Redeploy**（環境変数反映のため再デプロイ必須）
